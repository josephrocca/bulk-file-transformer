...
async function transform(inputData) {
  ...
  console.log(`worker processing file ${inputData.i}`);

  let inputImageBlob = await inputData.fileHandle.getFile();
  ...
  // ctx.save();
  // ctx.beginPath();
  // ctx.arc(75+256, 75, 60, 0, Math.PI*2, true);
  // ctx.closePath();
  // ctx.clip();
  // do stuff
  // ctx.restore();
  ...
  let offsetX = r()*256, offsetY = r()*128, sizeX = r()*100+50, sizeY = r()*100+50; // TODO: replace with proper center-based crop from inpaint_dataset_builder.js
  let imgChunk = ctx.getImageData(offsetX, offsetY, sizeX, sizeY);
  let w = imgChunk.width;
  let h = imgChunk.height;

  let styleImg = await fetch("http://127.0.0.1:3001/styles/style_4.jpg").then(r => r.blob()).then(blobToImageData);
  imgChunk = await styleTransferModel.stylize(imgChunk, styleImg, 1);
  
  let eachFxChance = 1;
  let fxTexture = fxCanvas.texture(imgChunk);
  fxCanvas.draw(fxTexture)
  // if(r() < eachFxChance) {
  //   let num = 1+Math.floor(r()*3);
  //   for(let i = 0; i < num; i++) {
  //     fxCanvas.swirl(w*r(), h*r(), Math.min(w, h)*r(), (r()-0.5)*3);
  //   }
  // }
  // if(r() < eachFxChance) {
  //   let num = 1+Math.floor(r()*3);
  //   for(let i = 0; i < num; i++) {
  //     fxCanvas.bulgePinch(w*r(), h*r(), Math.min(w, h)*r(), (r()-0.5)*2);
  //   }
  // }
  // if(r() < eachFxChance) {
  //   let quad = [0,0 , w,0 , 0,h , w,h];
  //   fxCanvas.perspective(quad, quad.map(n => n+rI(-20,20)));
  // }

  // random jagged/curved/rectangular crop/clip ?

  fxCanvas.update();
  
  imgChunk = await createImageBitmap(await fxCanvas.convertToBlob({type:"image/png"}));

  // let eachFilterChance = 0.2;
  // let filters = [
  //   `brightness(${r() < eachFilterChance ? 0.3+r(1.4) : 1})`,
  //   `contrast(${r() < eachFilterChance ? 0.3+r(5) : 1})`,
  //   `grayscale(${r() < eachFilterChance ? r() : 0})`,
  //   `hue-rotate(${r() < eachFilterChance ? Math.round(r(90)) : 0}deg)`,
  //   `invert(${r() < eachFilterChance ? r(0.5) : 0})`,
  //   `saturate(${r() < eachFilterChance ? r(4) : 1})`,
  // ];
  // ctx.filter = filters.map(f => [f,r()]).sort((a,b) => a[1]-b[1]).map(a => a[0]).join(" ");

  let offsetChangeX = (r()-0.5)*sizeX/10, offsetChangeY = (r()-0.5)*sizeY/10, sizeChangeX = (r()-0.5)*sizeX/10, sizeChangeY = (r()-0.5)*sizeY/10;

  ctx.save();
  ctx.translate(256+offsetX+offsetChangeX, 0+offsetY+offsetChangeY);
  ctx.rotate((r()-0.5)*0.2);
  ctx.drawImage(imgChunk, 0, 0, sizeX+sizeChangeX, sizeY+sizeChangeY);
  ctx.restore();

  return {
    blob: await canvas.convertToBlob({type: "image/jpeg"}),
    name: `${inputData.i}.jpg`,
  };
}
...
const blobToImageDataCanvas = new OffscreenCanvas(1,1);
const blobToImageDataCtx = blobToImageDataCanvas.getContext("2d");
async function blobToImageData(blob) {
  let imageBitmap = await createImageBitmap(blob);
  blobToImageDataCanvas.width = imageBitmap.width;
  blobToImageDataCanvas.height = imageBitmap.height;
  blobToImageDataCtx.drawImage(imageBitmap, 0, 0);
  return blobToImageDataCtx.getImageData(0, 0, imageBitmap.width, imageBitmap.height);
}


// I edited this to make it work in a WebWorker:
//   - replaced document.createElement('canvas') with new OffscreenCanvas(1,1)
//   - replaced canvas.getContext("experimental-webgl") with canvas.getContext("webgl")
// http://evanw.github.com/glfx.js/ (MIT Licensed, By Evan Wallace)
...
"$OES_texture_float_linear$",{enumerable:!1,configurable:!1,writable:!1,value:new d}),a=this.$OES_texture_float_linear$):a=n.call(this,a);return a}function e(){var a=f.call(this);-1===a.indexOf("OES_texture_float_linear")&&a.push("OES_texture_float_linear");return a}try{var g=new OffscreenCanvas(1,1).getContext("webgl")}catch(l){}if(g&&-1===g.getSupportedExtensions().indexOf("OES_texture_float_linear")&&a(g)){var n=WebGLRenderingContext.prototype.getExtension,f=WebGLRenderingContext.prototype.getSupportedExtensions;
WebGLRenderingContext.prototype.getExtension=c;WebGLRenderingContext.prototype.getSupportedExtensions=e}})();var a;v.canvas=function(){var b=new OffscreenCanvas(1,1);try{a=b.getContext("webgl",{premultipliedAlpha:!1})}catch(d){a=null}if(!a)throw"This browser does not support WebGL";b._={gl:a,isInitialized:!1,texture:null,spareTexture:null,flippedShader:null};b.texture=k(A);b.draw=k(C);b.update=k(D);b.replace=k(E);b.contents=k(F);b.getPixelArray=k(G);b.brightnessContrast=k(H);
